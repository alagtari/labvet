    ...
    if (localStorage.getItem("currentUser")) {
      this.userService.getInfo().subscribe(
        user => {
          ...
          if (user) {
            if (user['status'] == 200) {
              document.getElementById("name").innerHTML = user['data']['name']
              document.getElementById("role").innerHTML = user['data']['role']
              document.getElementById("img").setAttribute("src", user['data']['photo'])
            }
            else {
