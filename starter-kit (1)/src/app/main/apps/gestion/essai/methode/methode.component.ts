...
export class MethodeComponent implements OnInit {
  ...
  constructor(private modalservice: NgbModal, private _methodeService: MethodeService, private _coreConfigService: CoreConfigService, private _router: Router, private _toastr: ToastrService) {
  ...
  addMethode(modal) {
    ...
    this._methodeService.addMethode({ "designation": name, "id": 0 }).subscribe(result => {
      if (result) {
        if (result['status'] == 200) {
          this._toastr.success('Methode Ajoutée', 'Succès!', {
  ...
  deleteMethode(id) {
    ...
    console.log(id)
    this._methodeService.deleteMethode(id).subscribe(
      result => {
        if (result) {
          if (result['status'] == 200) {
            this._toastr.success('Methode supprimé', 'Succès!', {
              toastClass: 'toast ngx-toastr',
              closeButton: true
            });
  ...
  submit(form, modal) {
    ...
    if (form.valid) {
      ...
      this._methodeService.addMethode(form.form.value).subscribe(
        result => {
          if (result) {
            if (result['status'] == 200) {
              this._toastr.success('Methode Ajoutée', 'Succès!', {
    ...
    this._coreConfigService.config.pipe(takeUntil(this._unsubscribeAll)).subscribe(config => {
      ...
      if (config.layout.animation === 'zoomIn') {
        setTimeout(() => {
          ...
          this._methodeService.onUserListChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            console.log(this.rows)
            this.rows = response;
